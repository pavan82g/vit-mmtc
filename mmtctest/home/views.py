...
def show(request):
    ...
    if f == 1:
        ...
        report31 = pd.DataFrame()
        for m in k:
            r = pd.DataFrame(m)
            report3 = pd.concat([report3,r], axis = 0) 
        for m in range(0,len(report3)):
            p = list(report3.iloc[m,:])
            flightcon.append(p)
        for m in k1:
            r = pd.DataFrame(m)
            report31 = pd.concat([report31,r], axis = 0) 
        for m in range(0,len(report31)):
            p = list(report31.iloc[m,:])
            flightcon1.append(p)
    ...
    if h == 1:
        ...
        report41 = pd.DataFrame()
        for m in g:
            r = pd.DataFrame(m)
            report4 = pd.concat([report4,r], axis = 0) 
        for m in range(0,len(report4)):
            p = list(report4.iloc[m,:])
            traincon.append(p)
        for m in g1:
            r = pd.DataFrame(m)
            report4 = pd.concat([report41,r], axis = 0) 
        for m in range(0,len(report41)):
            p = list(report41.iloc[m,:])
            traincon1.append(p)


    return render(request,"home/result.html",{"t":t, "q":q, "f":flightcon, "f1":flightcon1, "tr":traincon, "tr1":traincon1})